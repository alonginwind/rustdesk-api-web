...
<script setup>
  ...
  // The controlled side reports the session kind using its own numeric codes
  // (0=Remote, 1=FileTransfer, 2=PortForward, 3=ViewCamera, 4=Terminal), which
  // differ from the protobuf ConnType enum (where RDP=3, VIEW_CAMERA=4,
  // TERMINAL=5).  The mapping below follows the codes the client actually
  // sends, not the protobuf definition.  An unmapped code still shows its
  // number, so a kind added later is visible rather than read as a desktop
  // session.  Zero is the absence of a report, which is why it maps to
  // nothing below and not to "#0".
  const connTypeKey = { 0: 'RemoteDesktop', 1: 'File', 2: 'PortForward', 3: 'ViewCamera', 4: 'Terminal' }
  const connTypeTag = { 0: 'primary', 1: 'warning', 2: 'info', 3: 'danger', 4: '' }
